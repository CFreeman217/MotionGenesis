function [t,VAR,Output] = bulletCd1
%===========================================================================
% File: bulletCd1.m created May 16 2019 by MotionGenesis 5.9.
...
BaseRadius=0; density=0; volumeBearing=0; volumeBoatTail=0; volumeOgive=0; volumeTotal=0; Cd=0; QxDt=0; QyDt=0; UxDt=0; UyDt=0;
...
bulletMass                      =  9.52544;                % kg                  Constant
g                               =  9.81;                   % m/s^2               Constant
oal                             =  0.028956;               % m                   Constant
ogiveLength                     =  0.0194818;              % m                   Constant
r                               =  0.00381;                % m                   Constant

Qx                              =  0;                      % m                   Initial Value
Qy                              =  0;                      % m                   Initial Value
Ux                              =  608.1118318204309;      % m/s                 Initial Value
Uy                              =  608.1118318204308;      % m/s                 Initial Value

tInitial                        =  0.0;                    % second              Initial Time
tFinal                          =  60.0;                   % second              Final Time
tStep                           =  0.001;                  % second              Integration Step
...
density = 0.001*bulletMass/volumeTotal;
...
QxDt = Ux;
QyDt = Uy;
Cd = 0.6366197723675814/(density*r^2*(Ux^2+Uy^2));
UxDt = -Cd*Ux/bulletMass;
UyDt = -g - Cd*Uy/bulletMass;
...
if( isempty(Output) ),
   if( ~isempty(FileIdentifier) ),
      ...
      fprintf( 1, '\n Output is in the files bulletCd1.i  (i=1,2)\n\n' );
   end
...
if( isempty(hasHeaderInformationBeenWritten) ),
   ...
   if( shouldPrintToFile && isempty(FileIdentifier) ),
      ...
      FileIdentifier(1) = fopen('bulletCd1.1', 'wt');   if( FileIdentifier(1) == -1 ), error('Error: unable to open file bulletCd1.1'); end
      fprintf(FileIdentifier(1), '%% FILE: bulletCd1.1\n%%\n' );
      fprintf(FileIdentifier(1), '%%      Qx             Qy\n' );
      fprintf(FileIdentifier(1), '%%      (m)            (m)\n\n' );
      FileIdentifier(2) = fopen('bulletCd1.2', 'wt');   if( FileIdentifier(2) == -1 ), error('Error: unable to open file bulletCd1.2'); end
      fprintf(FileIdentifier(2), '%% FILE: bulletCd1.2\n%%\n' );
      fprintf(FileIdentifier(2), '%%       t             Cd\n' );
      fprintf(FileIdentifier(2), '%%     (sec)        (noUnits)\n\n' );
   end
...
figure;
data = load( 'bulletCd1.1' ); 
plot( data(:,1),data(:,2),'-b', 'LineWidth',3 );
legend( 'Qy (m)' );
xlabel('Qx (m)');   ylabel('Qy (m)');   % title('Some plot title');
clear data;

figure;
data = load( 'bulletCd1.2' ); 
plot( data(:,1),data(:,2),'-b', 'LineWidth',3 );
legend( 'Cd (noUnits)' );
xlabel('t (sec)');   ylabel('Cd (noUnits)');   % title('Some plot title');
clear data;
end

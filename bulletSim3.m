function [t,VAR,Output] = bulletSim3
%===========================================================================
% File: bulletSim3.m created May 15 2019 by MotionGenesis 5.9.
...
QxDt = Ux;
QyDt = Uy;
UxDt = -coeffDrag*Ux/bulletMass;
UyDt = -g - coeffDrag*Uy/bulletMass;
...
if( isempty(Output) ),
   if( ~isempty(FileIdentifier) ),
      ...
      fprintf( 1, '\n Output is in the files bulletSim3.i  (i=1,2)\n\n' );
   end
...
if( isempty(hasHeaderInformationBeenWritten) ),
   ...
   if( shouldPrintToFile && isempty(FileIdentifier) ),
      ...
      FileIdentifier(1) = fopen('bulletSim3.1', 'wt');   if( FileIdentifier(1) == -1 ), error('Error: unable to open file bulletSim3.1'); end
      fprintf(FileIdentifier(1), '%% FILE: bulletSim3.1\n%%\n' );
      fprintf(FileIdentifier(1), '%%      Qx             Qy\n' );
      fprintf(FileIdentifier(1), '%%      (m)            (m)\n\n' );
      FileIdentifier(2) = fopen('bulletSim3.2', 'wt');   if( FileIdentifier(2) == -1 ), error('Error: unable to open file bulletSim3.2'); end
      fprintf(FileIdentifier(2), '%% FILE: bulletSim3.2\n%%\n' );
      fprintf(FileIdentifier(2), '%%       t        (Ux^2+Uy^2)^0.5\n' );
      fprintf(FileIdentifier(2), '%%     (sec)           (m/s)\n\n' );
   end
...
figure;
data = load( 'bulletSim3.2' ); 
plot( data(:,1),data(:,2),'-b', 'LineWidth',3 );
legend( '(Ux^2+Uy^2)^0.5 (m/s)' );
xlabel('t (sec)');   ylabel('(Ux^2+Uy^2)^0.5 (m/s)');   % title('Some plot title');
clear data;
end
...
%===================================
end    % End of function bulletSim3
%===================================
